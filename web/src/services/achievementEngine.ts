...
export const achievementEngine = {
    ...
    checkMasterOfModes(
        ...
        // Update progress for progress bar (sum, max 6)
        const totalProgress = Math.min(classicUnlocked, 3) + Math.min(rainbowUnlocked, 3);
        achievementService.updateProgress('master-modes', totalProgress);

        // Unlock if both are at least 3
        if (!progress['master-modes']?.unlocked && classicUnlocked >= 3 && rainbowUnlocked >= 3) {
